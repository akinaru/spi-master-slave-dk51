...
#define TX_RX_BUF_LENGTH         1                 /**< SPI transaction buffer length. */
...
static void increment_spi_counter(uint32_t err_code){
    SEGGER_RTT_printf(0, "%c - ",m_rx_data[0]);
...
static void spi_master_event_handler(nrf_drv_spi_event_t event)
{
    ...
    switch (event)
    {
        case NRF_DRV_SPI_EVENT_DONE:
            ...
            if (count == 0){
                if (m_rx_data[0]=='a'){
                    SEGGER_RTT_printf(0, "received response from slave :\n");
                    increment_spi_counter(err_code);
                }
            }
            else{
                increment_spi_counter(err_code);
...
static uint32_t send_spi_data_command(uint8_t * const p_tx_data,
    ...
    if (err_code!=NRF_SUCCESS){
        SEGGER_RTT_printf(0, "error occured on transfer :%d Resetting...\n",err_code);
        reset_spi_counter();
        //APP_ERROR_CHECK(err_code);
        return err_code;
    }
                          ...
                          const uint16_t  len)
{
    ...
    for (uint16_t i = 65;i  < 91;i++){
        p_tx_data[0]=i;
        uint32_t err_code = send_spi_data_command(p_tx_data,p_rx_data,len);
        if (err_code != NRF_SUCCESS){
            return;
        }
    }
    p_tx_data[0]='\n';
    send_spi_data_command(p_tx_data,p_rx_data,len);
}
